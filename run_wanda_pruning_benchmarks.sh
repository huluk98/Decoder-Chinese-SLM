...
run_config() {
  ...
  local config_path="$2"
  echo
  echo "=== ${label} Wanda pruning benchmark ==="
  echo "config: ${config_path}"
  MODE=generic PRUNING_METHODS=wanda CONFIG_PATH="${config_path}" ./scripts/run_pruning_benchmark_8way.sh "${config_path}"
}
...
echo
echo "Done."
echo "Regular SFT Wanda summary:      runs/pruning-benchmark-regular-sft-0p2b/benchmark_summary.csv"
echo "Contrastive SFT Wanda summary:  runs/pruning-benchmark-contrastive-sft-0p2b/benchmark_summary.csv"
