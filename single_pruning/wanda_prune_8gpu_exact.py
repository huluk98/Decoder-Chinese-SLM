#!/usr/bin/env python3
"""
Standalone 8-GPU WANDA-pruning + exact benchmark script for decoder-only HF models.

User command, only two paths:
    python wanda_prune_8gpu_exact.py /path/to/local_model /path/to/eval.json
...
What it does:
    ...
    4. Collects activation statistics and applies 50% WANDA pruning to selected nn.Linear weights.
...
def auto_launch_if_needed() -> None:
    ...
    if len(sys.argv) != 3:
        print("Usage: python wanda_prune_8gpu_exact.py /path/to/local_model /path/to/eval_file")
        sys.exit(2)
...
def make_output_dir(model_path: Path) -> Path:
    ...
    return model_path.parent / f"{model_path.name}_wanda50_8gpu_exact_{stamp}"
...
def _selected_linear_modules(model: nn.Module) -> List[Tuple[str, nn.Linear]]:
...
def build_calibration_batch(tokenizer, batch: List[EvalExample], device: torch.device) -> Dict[str, torch.Tensor]:
    """
    Build a causal-LM calibration batch for WANDA activation statistics.
    For prompt-response rows, use prompt + response so intermediate activations see the domain output format.
    For text-only rows, use text directly.
    """
    texts: List[str] = []
    for ex in batch:
        if ex.prompt is not None and ex.response is not None:
            texts.append(str(ex.prompt) + str(ex.response))
        elif ex.prompt is not None:
            texts.append(str(ex.prompt))
        else:
            texts.append(str(ex.text))
...
def collect_wanda_activation_sumsq(
    model: nn.Module,
    tokenizer,
    examples: List[EvalExample],
    rank: int,
    ...
    """
    Memory-safe WANDA calibration.

    Instead of storing all activations like the original T5 script, this accumulates
    sum(x^2) per input channel for each Linear layer. Across 8 GPUs, each rank sees
    a shard of the eval set and we all-reduce the activation statistics so every rank
    computes the same pruning mask.
    """
    selected = _selected_linear_modules(model)
    if not selected:
        raise RuntimeError("No 2D nn.Linear weights found for WANDA pruning.")
    ...
    def make_hook(module_name: str):
        def hook(module: nn.Module, inp: Tuple[torch.Tensor, ...], out: torch.Tensor):
            ...
            if not torch.is_tensor(x) or x.numel() == 0:
                return
            x = x.detach()
            # Linear input is normally [batch, seq, in_features] or [tokens, in_features].
            x = x.reshape(-1, x.shape[-1]).float()
            local = torch.sum(x * x, dim=0)
            if module_name not in sumsq:
                sumsq[module_name] = torch.zeros_like(local, device=device)
            sumsq[module_name].add_(local.to(device))
        return hook
    ...
    with torch.inference_mode():
        for batch_idx, batch in enumerate(loader):
            if batch_idx >= CALIB_BATCHES:
                break
            cal = build_calibration_batch(tokenizer, batch, device)
            # Labels are not required for WANDA; forward pass is enough to trigger Linear hooks.
            model(**cal, use_cache=False)

    for h in handles:
        h.remove()

    # Ensure every selected layer has a tensor, even if a rank saw no calibration batch.
    for name, module in selected:
        if name not in sumsq:
            sumsq[name] = torch.zeros(module.weight.shape[1], device=device, dtype=torch.float32)

    if world_size > 1:
        # Dict insertion order follows selected module order. All-reduce by name for clarity.
        for name, _ in selected:
            dist.all_reduce(sumsq[name], op=dist.ReduceOp.SUM)

    return sumsq


def wanda_prune_per_row(
    model: nn.Module,
    activation_sumsq: Dict[str, torch.Tensor],
    sparsity: float = SPARSITY,
) -> Tuple[Dict[str, torch.Tensor], List[Dict[str, Any]], List[Dict[str, Any]]]:
    """
    Same guideline as the uploaded WANDA script, adapted for decoder-only models:
      - collect activations for each nn.Linear
      - activation norm per input dimension = sqrt(sum x^2)
      - score = abs(W) * activation_norm
      - PER-ROW pruning: keep top (1 - sparsity) input weights in each output row
    """
    ...
    for name, m in model.named_modules():
        ...
        if m.weight is None or m.weight.ndim != 2:
            ...
            continue
        if name not in activation_sumsq:
            skipped.append({"module": name, "reason": "missing_activation_statistics"})
            continue

        with torch.no_grad():
            W = m.weight.data
            norm = torch.sqrt(activation_sumsq[name].to(device=W.device, dtype=torch.float32).clamp_min(0.0))
            if norm.numel() != W.shape[1]:
                skipped.append(
                    {
                        "module": name,
                        "reason": "activation_dimension_mismatch",
                        "weight_shape": list(W.shape),
                        "activation_dim": int(norm.numel()),
                    }
                )
                continue

            score = W.detach().abs().float() * norm.unsqueeze(0)
            keep = int(W.shape[1] * (1.0 - sparsity))
            if keep <= 0:
                skipped.append({"module": name, "reason": "keep_count_zero", "shape": list(W.shape)})
                continue
            if keep >= W.shape[1]:
                mask = torch.ones_like(W, dtype=torch.bool)
            else:
                topk = torch.topk(score, keep, dim=1, largest=True).indices
    ...
    if not rows:
        raise RuntimeError("No nn.Linear layers were pruned by WANDA.")
...
def main_worker() -> None:
    ...
    if is_rank0(rank):
        save_json(
            output_dir / "eval_dataset_summary.json",
            {
                ...
                "calibration_batches_per_rank": CALIB_BATCHES,
            },
        )
    ...
    log(rank, "[rank0] Collecting WANDA activation statistics from eval calibration batches...")
    activation_sumsq = collect_wanda_activation_sumsq(model, tokenizer, examples, rank, world_size, device)
    log(rank, "[rank0] Applying 50% WANDA per-row pruning to nn.Linear weights...")
    masks, sparsity_rows, skipped = wanda_prune_per_row(model, activation_sumsq, SPARSITY)
    ...
    if is_rank0(rank):
        report = {
            ...
            "rule": "WANDA: score=abs(weight)*sqrt(sum(input_activation^2)); per output row keep top 50%, zero remaining 50%",
            ...
            "after": after_stats,
            "notes": [
                "This follows the uploaded WANDA guideline but uses AutoModelForCausalLM instead of T5ForConditionalGeneration.",
                "It accumulates activation sum-of-squares instead of storing full activations, which is safer for 8-GPU runs.",
                "Across multiple GPUs, activation statistics are all-reduced so every rank computes the same pruning mask.",
                "Sparsity is targeted per selected Linear row; whole-model sparsity can be lower because embeddings/norms are not pruned.",
        ...
        torch.save(masks, output_dir / "wanda_pruning_masks.pt")
        write_csv(output_dir / "sparsity_by_module.csv", sparsity_rows)
        save_json(output_dir / "wanda_pruning_report.json", report)
        ...
        print("\n✅ WANDA pruning + exact benchmark complete")
